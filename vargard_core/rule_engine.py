...
class RuleEngine:
        ...
        for rule_config in rules_config:
            try:
                ...
                if self._validate_rule(rule):
                    ...
                    print(
                        f"Invalid rule: {rule_config.get('name',
                        'unnamed')}")
                    return False
            except Exception as e:
                print(
                    f"Error parsing rule {rule_config.get('name',
                    'unnamed')}: {e}")
                return False
    ...
    def _evaluate_conditions(
        ...
        if operator == LogicalOperator.AND:
            return all(
                self._evaluate_conditions(c,
                event_data) for c in conditions['conditions'])
    ...
    def _check_detection_condition(
        self,
        condition: Dict[str,
        Any],
        detection: Dict[str,
    ...
    def _compare_values(
        ...
        actual: Any,
        expected: Any,
        operator: ConditionOperator) -> bool:
